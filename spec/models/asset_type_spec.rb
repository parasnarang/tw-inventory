...
describe AssetType do
  ...
  context :unassigned_asset_count do
    ...
    it "should return num of assets that are yet not assigned to any employee" do
     FactoryGirl.create(:assignment, :asset => @asset1, :assignee => @non_admin, :start_date => 1.month.ago.to_date, :end_date => 1.month.from_now.to_date)
     @at.unassigned_asset_count.should == 1
    end

    it "should return 0 if all asset have been assigned" do
      FactoryGirl.create(:assignment, :asset => @asset1, :assignee => @non_admin, :start_date => 1.month.ago.to_date, :end_date => 1.month.from_now.to_date)
      FactoryGirl.create(:assignment, :asset => @asset2, :assignee => @non_admin, :start_date => 1.month.ago.to_date, :end_date => 1.month.from_now.to_date)
